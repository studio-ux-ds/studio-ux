...
- `DSIcon` resolves with `ALIASES[name] || name`, so an alias whose key matches a
  real glyph name silently overrides that glyph. Five entries did exactly that:
  `copy` → `file`, `history` → `refresh`, `upload` → `arrow-up-right`,
  `shield` → `lock` and `alert-triangle` → `alert-circle`. All five names exist
  as curated glyphs in `icons.js`, so "copy" rendered a document, "upload"
  rendered a diagonal arrow and every error state rendered a circle instead of
  the warning triangle.
- Removing the five aliases fixes the icon in `DashboardScreen`, `DetailScreen`,
  `FormScreen`, `ListScreen` and `SearchScreen` error states (`alert-triangle`),
  in the auditing route generated by the CLI (`history`), and in the Storybook
  examples for Perfis de acesso (`shield`), import (`upload`) and duplicate
  (`copy`). Consumers passing these names get the intended drawing with no code
  change.

### Guard — the build now rejects the whole class

- `check-packages.mjs` fails when an alias key matches a real glyph name, or when
  an alias points at a glyph that does not exist. This is the second occurrence
  of the same class: v1.2.15 fixed `square`/`square-check`/`square-minus` (the
  DataTable checkbox rendered a DOCUMENT) and left a warning comment in the file
  — the comment was not enough, so it is now enforced by the build.

### Note — 1.2.59 was burned, never published

- Tag `v1.2.59` was pushed from a working tree whose `git add`/`git commit` had
  failed on a stale `.git/index.lock`, so it points at the same commit as
  `v1.2.58` — package versions still `1.2.58`, aliases still broken. The
  tag-vs-package check in `check-packages.mjs` caught the mismatch and the
  publish workflow failed, so nothing wrong reached the registry. Tags are
  immutable: this release is `1.2.60`, and `1.2.59` stays burned.
