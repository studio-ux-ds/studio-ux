...
**PT** — `examples/referencia-aquapark.html` ilustra a jornada canônica de listagens (linha acionável com teclado → detalhe → criar/editar em página → confirmação destrutiva → retorno). Ele importa `packages/tokens/tokens.css` e `packages/components/components.css` reais; seus mapeamentos exclusivos de gráfico derivam de papéis semânticos, sem redefinir tokens nem componentes-base. É um molde de referência, não a prova de aprovação visual do DS: essa prova continua sendo o Laboratório visual integrado do Storybook, em claro e escuro. Telas AquaPark que ainda reproduzem a rota real com ações por linha devem migrar a jornada inteira no sistema de origem antes que a referência declare paridade final.

**EN** — `examples/referencia-aquapark.html` illustrates the canonical list journey (keyboard-accessible row → detail → page-based create/edit → destructive confirmation → return). It imports the real `packages/tokens/tokens.css` and `packages/components/components.css`; its example-only chart mappings derive from semantic roles without redefining tokens or base components. It is a reference template, not evidence of visual approval for the DS: that proof remains the integrated Storybook Visual Lab, in light and dark modes. AquaPark screens that still reproduce their real route with per-row actions must migrate the complete journey in the source system before the reference claims final parity.
