...
- **Dashboard grid (v1.2.50):** `DashboardScreen` usa `auto-fit` em `su-statgrid`
  ...
  AquaPark é o consumidor-canário desta correção.
...
- **Decisões visuais travadas / Locked visual decisions:** direção sóbria; **gradiente tonal e discreto é permitido** desde a v1.2.27 (uma matiz de token sobre a superfície, peça pequena — régua em `VISUAL_DNA` §6.4), gradiente saturado/multicor/de marca segue vetado; cor de ação configurável com paleta de acentos sóbrios; hierarquia e semântica vêm de tokens, não de literais na tela. / Sober direction; **tonal, discreet gradients are allowed** since v1.2.27 (one token hue over the surface, small piece — ruler in `VISUAL_DNA` §6.4), saturated/multi-hue/brand gradients remain vetoed; configurable action color with a sober accent palette; hierarchy and semantics come from tokens, never screen literals.
- **StatCard composition / Composição do StatCard:** semantic tones keep a neutral surface and carry their signal through border, value and optional icon; the Financial and Commercial references use that same component contract. / Tons semânticos mantêm a superfície neutra e levam seu sinal pela borda, valor e ícone opcional; as referências Financeiro e Comercial usam o mesmo contrato do componente.
...
| Intake de desenvolvimento · Development intake | Método (agnóstico) vive em `C:\Users\Flowspec\Documents\STUDIO-WORKFLOW\` (paradigma `prompt-framework/v1`, v3.0.0). Local: `prompt-framework/README.md` (stub apontando pro central), `prompt-framework/catalogo-especialidades.md` (`studio-ux.specialty-catalog@2.0.0`, `kind: shared`), `prompt-framework/especialidades/*` (6 especialidades no schema formal, namespace `studio-ux.*`, todas com `extends: workflow.system-change-base@1.0.0`). · Method (agnostic) lives in `STUDIO-WORKFLOW/` (`prompt-framework/v1` paradigm, v3.0.0). Local: stub + local catalog + 6 specialties on the formal schema. | ✅ |
