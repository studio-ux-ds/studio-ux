...
### 2.6 Laboratório visual integrado · Integrated visual lab
**PT** — *Oferece:* cenários completos e determinísticos, montados exclusivamente com o Runtime oficial, para avaliar a linguagem visual antes de uma adoção em sistema consumidor. O cenário inclui casca, dados, formulário e estados fora do caminho feliz; não é uma maquete nem uma cópia de uma Application. *Quando usar:* antes de promover refinamento visual de tokens, CSS ou padrões para AquaPark, IA Studio, Finanças, Delivery ou outro consumidor. *Consome:* `@studio-ux-ds/react`, `tokens.css` e `components.css` reais. A primeira implementação é a story **`Referência visual/Laboratório financeiro`** no Storybook: `AppShell`, dashboard, `ListScreen`, formulário e estados de referência. O menu do usuário abre `Customize`, onde a validação também deve cobrir Tema, Accent, Sidebar/Top Nav e `pt-BR`/`en`.

**EN** — *Offers:* complete deterministic scenarios, assembled exclusively with the official Runtime, for evaluating the visual language before adoption in a consumer system. The scenario includes shell, data, form and unhappy-path states; it is neither a mockup nor a copy of an Application. *When:* before promoting visual refinement of tokens, CSS or patterns to AquaPark, IA Studio, Finanças, Delivery or another consumer. *Consumes:* the real `@studio-ux-ds/react`, `tokens.css` and `components.css`. The first implementation is the **`Referência visual/Laboratório financeiro`** Storybook story: `AppShell`, dashboard, `ListScreen`, form and reference states. The user menu opens `Customize`, where validation must also cover Theme, Accent, Sidebar/Top Nav and `pt-BR`/`en`.
...
## 6. Regra de aprovação visual · Visual approval rule
**PT** — Uma frente que altera a linguagem visual (tokens, elevação, raio, tipografia, densidade, superfícies, controles ou casca) é avaliada primeiro no Laboratório visual integrado, em claro e escuro. Nenhum consumidor é o lugar de descobrir ou compensar uma decisão de design ainda não aprovada. O resultado da revisão pode promover uma mudança ao dono correto — tokens, CSS ou adapter — e o Laboratório então mostra essa mudança automaticamente. Depois da aprovação, uma adoção-canário confirma a integração no sistema; ela não substitui a validação visual do DS.

**EN** — A workstream that changes the visual language (tokens, elevation, radius, typography, density, surfaces, controls or shell) is evaluated first in the integrated Visual Lab, in light and dark modes. No consumer is the place to discover or compensate for a design decision that has not been approved. The outcome of the review may promote a change to the right owner — tokens, CSS or adapter — and the Lab then shows that change automatically. After approval, a canary adoption confirms integration in a system; it does not replace DS visual validation.
