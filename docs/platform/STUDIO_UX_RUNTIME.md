...
Complementa · Complements:   PLATFORM, ARCHITECTURE, PACKAGES, CONSTITUTION, VERSIONING, tools/CLI, tools/PLAYGROUND.
Nunca substitui · Never replaces: ARCHITECTURE (mapa de domínios), PACKAGES (layout físico), nem os donos
                             internos do Playground (Storybook em `.storybook/` + `stories/`), dos Exporters
                             (`packages/tokens/exports/`) e dos moldes de tela (`packages/react/patterns/`).
                             / ARCHITECTURE (domain map), PACKAGES (physical layout), and the internal owners of
                             the Playground (Storybook in `.storybook/` + `stories/`), the Exporters
                             (`packages/tokens/exports/`) and the screen molds (`packages/react/patterns/`).
...
**PT** — O **Playground** é o ambiente de *demonstração e teste vivo* dos componentes: onde cada peça do Runtime aparece em todos os seus estados (default/hover/focus/active/disabled/loading/error/empty, §8) para inspeção, calibragem e verificação de conformidade. É uma camada **derivada** — consome o Runtime, nunca o define. **Materialização atual:** o Playground vigente é o **Storybook** do monorepo (`.storybook/main.js` + `stories/*.stories.jsx|.mdx`), publicado como site estático pelo workflow `.github/workflows/storybook.yml` (GitHub Pages). Cada story documenta um componente do `@studio-ux-ds/react` real (lendo o `.jsx` antes — regra de `CLAUDE.md`), nunca uma API suposta. O dono operacional continua sendo `docs/tools/STUDIO_UX_PLAYGROUND.md`. O Playground não é destino de produção: nenhuma Application consome o Playground, ela consome o Runtime.
**EN** — The **Playground** is the *live demonstration and testing* environment for components: where each Runtime piece appears in all its states (default/hover/focus/active/disabled/loading/error/empty, §8) for inspection, calibration and conformance checking. It is a **derived** layer — it consumes the Runtime, never defines it. **Current materialization:** today's Playground is the monorepo's **Storybook** (`.storybook/main.js` + `stories/*.stories.jsx|.mdx`), published as a static site by the `.github/workflows/storybook.yml` workflow (GitHub Pages). Each story documents a real `@studio-ux-ds/react` component (reading the `.jsx` first — `CLAUDE.md` rule), never an assumed API. The operational owner remains `docs/tools/STUDIO_UX_PLAYGROUND.md`. The Playground is not a production destination: no Application consumes the Playground; it consumes the Runtime.

### 1.4 Templates — os moldes · the molds
**PT** — **Templates** são *moldes de tela e de projeto* prontos — composições oficiais (uma casca, uma tela de lista, um cabeçalho de página, um painel de preferências) construídas sobre o Runtime e a gramática (`GRAMMAR`), servindo de ponto de partida conforme. É uma camada **derivada**: um Template não inventa regra, ele *aplica* componentes e padrões já existentes. **Materialização atual:** os moldes de tela vivem em `packages/react/patterns/*.jsx` (`AppShell`, `PageHeader`, `ListScreen`, `AuthScreen`, `MessageScreen`, `Customize`) e são descritos como spec no **Grupo G** de `docs/components/STUDIO_UX_COMPONENT_LIBRARY.md`; a geração de projeto/tela por CLI é acionada por `studio generate` (`packages/cli/generator/`) e tem dono próprio em `docs/generation/STUDIO_UX_PROJECT_GENERATOR.md`. Um Template consome o Runtime; a Application instancia o Template e depois preenche com seu conteúdo de negócio (Art. 19).
**EN** — **Templates** are ready *screen and project molds* — official compositions (a shell, a list screen, a page header, a preferences panel) built on the Runtime and the grammar (`GRAMMAR`), serving as a compliant starting point. It is a **derived** layer: a Template invents no rule, it *applies* existing components and patterns. **Current materialization:** the screen molds live in `packages/react/patterns/*.jsx` (`AppShell`, `PageHeader`, `ListScreen`, `AuthScreen`, `MessageScreen`, `Customize`) and are described as spec in **Group G** of `docs/components/STUDIO_UX_COMPONENT_LIBRARY.md`; project/screen generation via CLI is driven by `studio generate` (`packages/cli/generator/`) and has its own owner in `docs/generation/STUDIO_UX_PROJECT_GENERATOR.md`. A Template consumes the Runtime; the Application instantiates the Template and then fills it with its business content (Art. 19).
...
**PT** — Depende da `CONSTITUTION` (Art. 1, 5, 13, 14, 19) e do `STUDIO_UX.md` (§7, §13). Serve o `ARCHITECTURE` (que mapeia os domínios que estas camadas executam) e o `PACKAGES` (que dá endereço físico a Specification, Runtime, Playground e Templates). Os Exporters (`packages/tokens/exports/` — Tailwind preset, JS theme, JSON, W3C DTCG, Figma) e o Project Generator (`packages/cli/generator/`, verbo `studio generate`) são os mecanismos que produzem o Runtime e os Templates a partir da Specification.
**EN** — Depends on the `CONSTITUTION` (Art. 1, 5, 13, 14, 19) and `STUDIO_UX.md` (§7, §13). Serves `ARCHITECTURE` (which maps the domains these layers execute) and `PACKAGES` (which gives Specification, Runtime, Playground and Templates a physical address). The Exporters (`packages/tokens/exports/` — Tailwind preset, JS theme, JSON, W3C DTCG, Figma) and the Project Generator (`packages/cli/generator/`, verb `studio generate`) are the mechanisms that produce the Runtime and Templates from the Specification.
...
## Roadmap
**PT** — As cinco camadas já existem em código. **Specification:** `STUDIO_UX.md` + `docs/**` (permanente, agnóstica). **Runtime:** `@studio-ux-ds/tokens` + `@studio-ux-ds/components` + `@studio-ux-ds/mobile` + `@studio-ux-ds/react` + `@studio-ux-ds/react-native` + `@studio-ux-ds/icons` — descartável e substituível por tecnologia (P4). **Playground:** Storybook em `.storybook/` + `stories/`. **Templates:** `packages/react/patterns/*.jsx` + `packages/cli/generator/`. **Applications:** os sistemas do Robson (AquaPark, IA Studio, Finanças, Delivery). Este documento evolui quando uma nova tecnologia-alvo entra (novo Runtime, ex.: um adapter para outra plataforma) ou quando um novo epic cria um Runtime paralelo — nunca quando um único componente muda. Fases históricas e Definição de Pronto por fase vivem em `docs/STUDIO_UX_ROADMAP.md`.
**EN** — All five layers already exist in code. **Specification:** `STUDIO_UX.md` + `docs/**` (permanent, agnostic). **Runtime:** `@studio-ux-ds/tokens` + `@studio-ux-ds/components` + `@studio-ux-ds/mobile` + `@studio-ux-ds/react` + `@studio-ux-ds/react-native` + `@studio-ux-ds/icons` — disposable and replaceable per technology (P4). **Playground:** Storybook in `.storybook/` + `stories/`. **Templates:** `packages/react/patterns/*.jsx` + `packages/cli/generator/`. **Applications:** Robson's systems (AquaPark, IA Studio, Finanças, Delivery). This document evolves when a new target technology arrives (a new Runtime, e.g. an adapter for another platform) or when a new epic creates a parallel Runtime — never when a single component changes. Historical phases and per-phase Definition of Done live in `docs/STUDIO_UX_ROADMAP.md`.
