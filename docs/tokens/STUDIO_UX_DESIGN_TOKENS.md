...
## 5. Arquitetura precede — e agora ancora — os valores concretos · Architecture precedes — and now anchors — the concrete values

**PT** — Este documento — e todos os de token — define **arquitetura, camadas, nomenclatura e regras**. Os valores concretos (paleta, hex, tipografia, px da escala) foram fixados na **Fase 2** (aprovados 2026-07-15) e vivem hoje como fonte executável em `packages/tokens/tokens.css` (variáveis `--su-*`, congeladas desde `v1.0.0`) mais os `exports/` (Tailwind preset, JS theme, JSON, W3C DTCG, Figma). Os valores materializados por família estão listados: cor em `STUDIO_UX_COLOR_SYSTEM.md` §9, tipografia em `STUDIO_UX_TYPOGRAPHY.md`, espaço em `STUDIO_UX_SPACING.md`, e as demais famílias (raio, elevação, motion, breakpoints, ícone, opacidade, z-index) no bloco "Valores materializados" ao final deste doc. **A ordem permanente é a mesma:** primeiro a estrutura (camadas, papéis, coesão), depois os valores — nunca ao contrário ("arquitetura antes de estética", `STUDIO_UX.md` §3, regra 6). Alterar um valor materializado segue o SemVer do §8; **evoluir a arquitetura** exige ADR curto no doc dono antes.

**EN** — This document — and every token doc — defines **architecture, layers, naming and rules**. The concrete values (palette, hex, typography, scale pixels) were fixed in **Phase 2** (approved 2026-07-15) and live today as an executable source in `packages/tokens/tokens.css` (`--su-*` variables, frozen since `v1.0.0`) plus `exports/` (Tailwind preset, JS theme, JSON, W3C DTCG, Figma). Materialized values per family are listed: color in `STUDIO_UX_COLOR_SYSTEM.md` §9, typography in `STUDIO_UX_TYPOGRAPHY.md`, spacing in `STUDIO_UX_SPACING.md`, and the remaining families (radius, elevation, motion, breakpoints, icon size, opacity, z-index) in the "Materialized values" block at the end of this doc. **The permanent order is the same:** structure first (layers, roles, cohesion), values second — never the other way around ("architecture before aesthetics", `STUDIO_UX.md` §3, rule 6). Changing a materialized value follows the SemVer of §8; **evolving the architecture** requires a short ADR in the owning doc first.
