...
### P26 — Dado de leitura se distribui; não desce numa coluna só · Read-only data distributes; it does not run down a single column
**PT** — Conjunto de pares rótulo↔valor (ficha de detalhe, resumo de confirmação, cabeçalho de registro) se distribui pela largura disponível, com piso de coluna, do mesmo modo que campos de formulário. Uma fileira única num container largo é defeito, não escolha: gasta uma fração da largura, empurra a tela para a rolagem e cria pressão falsa por abas para "caber". Duas consequências obrigatórias: **(a)** rótulo e valor ficam ADJACENTES — nunca separados por `space-between`, que num container largo deixa mil pixels entre a pergunta e a resposta; **(b)** antes de dividir uma tela de detalhe em abas, verificar se o conteúdo não cabe distribuído — abas para compensar layout desperdiçado escondem informação sem necessidade (P14 vale para o que está presente, não só para o vazio). Coluna única é a EXCEÇÃO, pedida explicitamente, para painel estreito ou resumo lateral. Usar `DescriptionList` — ficha montada à mão não recebe refinamento do DS e volta a divergir.
**EN** — A set of label↔value pairs (detail card, confirmation summary, record header) distributes across the available width with a column floor, the same way form fields do. A single file in a wide container is a defect, not a choice: it spends a fraction of the width, pushes the screen into scrolling and creates false pressure for tabs to "fit". Two required consequences: **(a)** label and value stay ADJACENT — never separated by `space-between`, which in a wide container leaves a thousand pixels between the question and the answer; **(b)** before splitting a detail screen into tabs, check whether the content fits once distributed — tabs that compensate for wasted layout hide information needlessly. A single column is the EXCEPTION, explicitly requested, for a narrow panel or side summary. Use `DescriptionList` — a hand-built card receives no DS refinement and drifts again.
