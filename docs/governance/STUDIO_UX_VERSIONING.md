...
## 2. Convenção vigente da era de documentação · Current documentation-era convention
**PT** — Decisão do Robson (registrada no `HANDOFF`): a `v0.2.0` permanece (tag imutável, número que ficou errado mas não se reusa), e enquanto durar a **era de documentação** (Fases 1.5–1.6 e a plataforma), varia-se apenas o dígito final na faixa `v0.2.1 … v0.2.99`. O salto para `v1.0.0` fica reservado ao **congelamento dos tokens** (Fase 2) — o primeiro contrato estável que um consumidor pode declarar.
**EN** — Robson's decision (recorded in `HANDOFF`): `v0.2.0` stays (immutable tag, a number that turned out wrong but is never reused), and while the **documentation era** lasts (Phases 1.5–1.6 and the platform), only the final digit varies within `v0.2.1 … v0.2.99`. The jump to `v1.0.0` is reserved for **freezing the tokens** (Phase 2) — the first stable contract a consumer can declare.
