...
**PT** — As linhas já percorridas (imutáveis): `v0.2.x` (documentação) → `v0.3.0…v0.3.99` (Fundação congelada) → `v1.0.0` (tokens congelados) → `v1.1.x` → **`v1.2.x` (atual, após a tag publicada `v1.2.0`)**. Cada linha anda pelo último dígito até o Robson decidir o próximo salto. A mudança para `v1.2.0` não altera a regra: a próxima tag é `v1.2.1`, e saltos futuros continuam exigindo decisão explícita.
**EN** — Lines already travelled (immutable): `v0.2.x` (docs) → `v0.3.0…v0.3.99` (frozen Foundation) → `v1.0.0` → `v1.1.x` → **`v1.2.x` (current, after published tag `v1.2.0`)**. Each line advances by its last digit until Robson decides the next jump. The move to `v1.2.0` does not alter the rule: the next tag is `v1.2.1`, and future jumps still require an explicit decision.
