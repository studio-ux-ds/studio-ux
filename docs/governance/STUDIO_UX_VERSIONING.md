...
**PT — CONVENÇÃO DE TREM DE RELEASE (regra do Robson; é ESTA que vale no dia a dia, acima da leitura genérica de SemVer do §1).** Incrementa-se **SOMENTE o último dígito da linha ativa**. A próxima tag é sempre a atual **+1 no último dígito** (ex.: `v1.1.0 → v1.1.1 → v1.1.2 … → v1.1.99`). **Mudar o 2º dígito (MINOR) ou o 1º (MAJOR) NÃO é automático e NÃO se deduz de SemVer** ("isso é feature nova, logo MINOR" é **suposição proibida** — Art. 20/21): um salto de linha só acontece por **decisão explícita do Robson**. A IA nunca promove a versão sozinha. Sempre confirmar o número com o Robson antes de taggear; a tag é **imutável e nunca reusada** (se subiu ao remote, seguiu — não se volta a um dígito menor, que quebraria a ordem). **Pós-compactação:** re-ancorar a versão lendo `git tag --sort=-v:refname` (o estado REAL), nunca de memória. Linhas já percorridas (imutáveis): `v0.2.x` (documentação) → `v0.3.0…v0.3.99` (Fundação congelada) → `v1.0.0` (tokens congelados) → **`v1.x` (atual)**.
**EN — RELEASE-TRAIN CONVENTION (Robson's rule; THIS governs day-to-day, above the generic SemVer reading in §1).** Only the **last digit of the active line** increments; the next tag is always current **+1 on the last digit** (`v1.1.0 → v1.1.1 → …`). **Bumping MINOR or MAJOR is NOT automatic and is NOT inferred from SemVer** ("it's a feature, therefore MINOR" is a **forbidden assumption** — Art. 20/21): a line jump happens only by Robson's explicit decision. The AI never promotes the version by itself. Always confirm the number with Robson before tagging; tags are immutable and never reused (once pushed, move forward — never drop to a lower digit, which breaks ordering). **After a compaction:** re-anchor the version from `git tag --sort=-v:refname` (the REAL state), never from memory.
