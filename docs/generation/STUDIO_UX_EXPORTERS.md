...
**PT** — **Implementado** em `packages/cli/exporters/export-tokens.mjs` (rodar: `npm run export:tokens` ou `studio export`), lendo a fonte de tokens e emitindo os artefatos. **Consumer-side (v1.1.15):** a fonte é resolvida por precedência `--tokens`/`STUDIO_UX_TOKENS` → pacote instalado `@studio-ux-ds/tokens` → monorepo (fallback); a saída vai para `--out` ou, por padrão, `packages/tokens/exports/` no monorepo e `./studio-ux-tokens` num projeto consumidor. A versão do carimbo vem do pacote de tokens ao lado da fonte. **Os 10 alvos da tabela §2 estão emitidos.** Web/interop **validados por máquina** (v1.1.2): JSON, Design Tokens (W3C/DTCG), Figma Tokens, Tailwind (preset `su-*`), tema JS (React + React Native), CSS Variables. Nativos **gerados e validados estruturalmente** (v1.1.3 — contagem/cobertura, chaves balanceadas), mas **não compilados no ambiente do DS** (sem dart/swiftc/kotlinc) — o produto confirma compilando: Flutter (`tokens.dart`), SwiftUI (`Tokens.swift`), Compose (`Tokens.kt`). Fidelidade garantida por: direção única, determinismo (mesma fonte → mesmo artefato) e cobertura (o script aborta se o tema escuro introduzir token inexistente no claro). Ver `tools/exporters/README.md`.
