...
**Camada React (runtime).** Além do gerador HTML (`templates.mjs`), **3 dos 9 moldes** também são materializados como componentes React em `@studio-ux-ds/react`, para apps React instanciarem o molde **em runtime** (não só via `studio generate`):

| Molde | React (runtime) | Desde |
|---|---|---|
| `list` | `patterns/PageHeader` + `patterns/ListScreen` | v1.1.x |
| `login` | `patterns/AuthScreen` (+ `AuthLink`) | v1.2.40 |
| `empty` (tela inteira) | `patterns/MessageScreen` | v1.2.40 |

O `ListScreen` tem a mesma composição do gerador (cabeçalho + filtros/busca no header do card + `DataTable` no Desktop / `Card`s em tela estreita + paginação + estados), derivada de `TABLES`; header no padrão Flux. O `AuthScreen` é o porte fiel de `examples/login.html` (painel de marca à esquerda só a partir de 900px, formulário `max-width: 380px`, marca absoluta no topo) — o painel só é renderizado quando há conteúdo para ele, o que cobre também os acessos obrigatórios (aceitar convite, redefinir senha) onde não há o que argumentar. O `MessageScreen` cobre os quatro casos concretos do molde `empty` de tela inteira — endereço que não existe, acesso que a pessoa não tem, recurso removido, erro inesperado — e carrega a regra de que **nenhum deles redireciona em silêncio**.

**Lição que produziu esta tabela (v1.2.40).** Os moldes estavam especificados (§2) e materializados em HTML, mas só o `list` existia em React. O resultado prático: um app consumidor migrou a interface inteira e **escreveu a tela de login à mão**, sem painel de marca, sem "esqueci a senha", sem "manter conectado" — não por decisão de desenho, mas porque nada o obrigou a encontrar o molde. Especificação sem materialização na camada que o consumidor usa **não é adotada**: ela existe para quem já sabe que existe. Molde novo em React entra quando um consumidor chega nele, e a tabela acima é o inventário obrigatório antes de escrever qualquer tela de classe recorrente (Regra Zero).
**EN** — Materialized in `packages/cli/generator/templates.mjs` (v1.1.10), triggered by `studio generate --mold <name> --into <project>`. **9 templates** (§2). Each **composes only catalog `.su-*` classes** (verified: every emitted class exists in `components.css`) with empty content points (`su-empty`, Art. 19); no token or piece is redefined (§3). `list` has a per-product variant (table on Desktop, cards on Mobile). **React layer:** 3 of the 9 templates are also materialized as React components — `list` (`PageHeader` + `ListScreen`), `login` (`AuthScreen` + `AuthLink`, v1.2.40) and full-screen `empty` (`MessageScreen`, v1.2.40) — so React apps can instantiate the template at runtime. Lesson behind that: a spec materialized only in HTML **is not adopted** by a React consumer; one of them migrated its whole UI and hand-wrote the login screen because nothing forced it to find the template. The screen is a browser-openable HTML (imports tokens+pieces from `node_modules`), giving immediate visual proof. **Honest guard:** refuses to generate outside a `studio create` project. `studio generate --list` shows the catalog.
