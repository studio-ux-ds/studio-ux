...
**Camada React (runtime).** Além do gerador HTML (`templates.mjs`), **os 9 moldes** são materializados como componentes React em `@studio-ux-ds/react` (desde a v1.2.46), para apps React instanciarem o molde **em runtime** e não só via `studio generate`:

| Molde | React (runtime) | Desde |
|---|---|---|
| `list` | `ListScreen` (+ `PageHeader`) | v1.1.x |
| `login` | `AuthScreen` (+ `AuthLink`) | v1.2.40 |
| `empty` (tela inteira) | `MessageScreen` | v1.2.40 |
| `detail` | `DetailScreen` | v1.2.46 |
| `form` | `FormScreen` | v1.2.46 |
| `dashboard` | `DashboardScreen` | v1.2.46 |
| `search` | `SearchScreen` | v1.2.46 |
| `settings` | `SettingsScreen` (+ `SettingsSection`) | v1.2.46 |
| `wizard` | `WizardScreen` | v1.2.46 |

Cada um tem a spec completa no **Grupo G** de `components/COMPONENT_LIBRARY.md` (quando usar, quando NÃO usar, regras, estados, anti-padrões) e uma story com os estados reais. O `AuthScreen` é o porte fiel de `examples/login.html`. O `ListScreen` tem a mesma composição do gerador, derivada de `TABLES`.

**A regra do "quando NÃO usar" é parte do molde.** Materializar um molde sem ela produz o efeito oposto ao pretendido: `WizardScreen` usado onde os campos são independentes esconde metade das perguntas; `SearchScreen` usado para filtrar uma lista duplica a busca da Toolbar; `FormScreen` dentro de um Modal quebra a regra dos três containers. Um molde disponível e mal escolhido é pior que nenhum molde, porque parece certo.

**Lição que produziu esta tabela (v1.2.40 → v1.2.46).** Os moldes estavam especificados (§2) e materializados em HTML, mas por muito tempo só o `list` existia em React. O resultado prático: um app consumidor migrou a interface inteira e **escreveu a tela de login à mão** — sem painel de marca, sem "esqueci a senha", sem "manter conectado" — não por decisão de desenho, mas porque nada o obrigou a encontrar o molde. Depois disso a medição mostrou o tamanho real do buraco no mesmo consumidor: **18 telas de detalhe/editor, 16 formulários, 1 passo a passo e 3 painéis**, todos arranjados à mão, cada um com o seu espaçamento, o seu jeito de mostrar carregando e a sua decisão sobre onde fica o "voltar".

Especificação sem materialização na camada que o consumidor usa **não é adotada** — ela existe para quem já sabe que existe. Esta tabela é o inventário obrigatório antes de escrever qualquer tela de classe recorrente (Regra Zero), e agora ela não tem linha vazia.
**EN** — Materialized in `packages/cli/generator/templates.mjs` (v1.1.10), triggered by `studio generate --mold <name> --into <project>`. **9 templates** (§2). Each **composes only catalog `.su-*` classes** (verified: every emitted class exists in `components.css`) with empty content points (`su-empty`, Art. 19); no token or piece is redefined (§3). `list` has a per-product variant (table on Desktop, cards on Mobile). **React layer:** since v1.2.46 **all 9 templates** are materialized as React components (`ListScreen`, `AuthScreen`, `MessageScreen`, `DetailScreen`, `FormScreen`, `DashboardScreen`, `SearchScreen`, `SettingsScreen`, `WizardScreen`), so React apps can instantiate a template at runtime. Lesson behind that: a spec materialized only in HTML **is not adopted** by a React consumer; one of them migrated its whole UI and hand-wrote the login screen because nothing forced it to find the template — and the follow-up measurement found 18 detail screens, 16 forms, 1 wizard and 3 panels in the same state. **The "when NOT to use" rule is part of the template:** an available, wrongly chosen template is worse than none, because it looks right. The screen is a browser-openable HTML (imports tokens+pieces from `node_modules`), giving immediate visual proof. **Honest guard:** refuses to generate outside a `studio create` project. `studio generate --list` shows the catalog.
